...
-(id)initWithURL:(NSURL *)videoURL quality:(LBYouTubeVideoQuality)videoQuality {
    ...
    if (self) {
        ...
        self.extractionExpression = [self readExtractionExpression];
    }
    return self;
}
...
-(NSString *) readExtractionExpression {
    NSString *result = @"(?<=\\\\\")http[^\"]*?itag=[^\"]*?(?=\\\\\")";
    NSString *localPath = [[[NSFileManager defaultManager] applicationSupportDirectory] stringByAppendingPathComponent:@"youtube.txt"];
    NSError *error;
